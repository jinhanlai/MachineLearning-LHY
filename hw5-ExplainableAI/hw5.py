...
if __name__ == '__main__':
    dataset_path = "../data/hw3/food-11/"  # hw3根hw5使用的是一样的数据
    ...
    train_paths, train_labels = get_paths_labels(os.path.join(dataset_path, 'training'))
    ...
    img_indices = [83, 4218, 4707, 8598] # 图片的索引
    images, labels = train_set.getbatch(img_indices)
    saliencies = compute_saliency_maps(images, labels, model) # 把图片输入进模型，获得图片的梯度，然后进行标准化
    ...
    filter_activations, filter_visualization = filter_explaination(images, model, cnnid=15, filterid=0, iteration=100,
                                                             lr=0.1)
